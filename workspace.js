...
cpdefine("inline:com-chilipeppr-workspace-sparkys-grbl", ["chilipeppr_ready"], function() {
    return {
        /**
         * The ID of the widget. You must define this and make it unique.
         */
        id: "com-chilipeppr-workspace-sparkys-grbl", // Make the id the same as the cpdefine id
        name: "Workspace / expanded to support xy touchplate zeroing", // The descriptive name of your widget.
        ...
        loadWidgets: function(callback) {
            ...
            var touchPlateYObj = {
                touchPlateYBtn: null,
                touchPlateYDiv: null,
                touchPlateYInstance: null,
                init: function() {
                    this.touchPlateYBtn = $('#com-chilipeppr-ws-gcode-menu .touchplatey-button');
                    this.touchPlateYDiv = $('#com-chilipeppr-ws-touchplatey');
                    this.setupBtn();
                    console.log("done instantiating touchPlatey add-on widget");
                },
                setupBtn: function() {
                    this.touchPlateYBtn.click(this.toggletouchPlateY.bind(this));
                },
                toggletouchPlateY: function() {
                    if (this.touchPlateYDiv.hasClass("hidden")) {
                        // unhide
                        this.showtouchPlateY();
                    }
                    else {
                        this.hidetouchPlateY();
                    }
                },
                showtouchPlateY: function(callback) {
                    this.touchPlateYDiv.removeClass("hidden");
                    this.touchPlateYBtn.addClass("active");

                    // see if instantiated already
                    // if so, just activate
                    if (this.touchPlateYInstance != null) {
                        //this.gpioInstance.activateWidget();
                        if (callback) callback();
                    }
                    else {
                        ...
                        var that = this;
                        chilipeppr.load(
                            "#com-chilipeppr-ws-touchplatey",
                            "http://fiddle.jshell.net/SparkysWIdgets/mLnfnL9t/show/light/",
                            function() {
                                require(["inline:com-chilipeppr-widget-touchplatey"], function(touchPlateY) {
                                    that.touchPlateYInstance = touchPlateY;
                                    console.log("touchPlateY instantiated. touchPlateYInstance:", that.touchPlateYInstance);
                                    that.touchPlateYInstance.init();
                                    //eagleInstance.activateWidget();
                                    if (callback) callback();
                                });
                            }
                ...
                hidetouchPlateY: function() {
                    this.touchPlateYDiv.addClass("hidden");
                    this.touchPlateYBtn.removeClass("active");
                    if (this.touchPlateYInstance != null) {
                        //this.gpioInstance.unactivateWidget();
                    }
                    $(window).trigger('resize');
